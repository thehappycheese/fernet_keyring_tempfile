...
class FernetKeyringTempfile:
    ...
    def get_or_create_key(self) -> bytes:
        try:
            key_existing = keyring.get_password(self.get_keyring_service_name(), "key")
            if key_existing is not None:
                return key_existing.encode("utf-8")
            else:
                key_fresh = Fernet.generate_key()
                keyring.set_password(self.get_keyring_service_name(), "key", key_fresh.decode("utf-8"))
                return key_fresh
        except PasswordSetError as e:
            raise ValueError("Unable to set password using keyring for some reason") from e
    ...
    def load(self) -> bytes:
        with self.get_temp_file_path().open("rb") as temp_file:
            ...
            data_plaintext = Fernet(key_existing.encode("utf-8")).decrypt(data_encrypted)
    ...
    def remove(self, missing_ok=True) -> None:
        ...
        try:
            keyring.delete_password(self.get_keyring_service_name(), "key")
        except PasswordDeleteError as e:
            if not missing_ok:
                raise ValueError("Unable to delete password using keyring for some reason") from e
